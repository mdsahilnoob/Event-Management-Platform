    ...
    <div className={inter.className}>
      ...
      <div className="bg-light dark:bg-dark px-5 md:px-0 min-h-screen h-fit text-dark dark:text-light overflow-x-hidden transition duration-100 ease-in-out pb-5">
        <div className="flex md:mt-44 px-20 justify-between md:flex-row flex-col-reverse md:items-start items-center md:gap-0 gap-10 mt-20">
          {loginNews.map((news) => (
            <div key={news.name} className="md:mt-10 mt-0 md:w-1/2">
              <NewsCards
                name={news.name}
                image={news.image}
                title={news.title}
          ...
          <form
            ...
            className="w-[90vw] md:w-1/2 flex flex-col gap-10 items-center justify-center md:border-none border-dark/50 border-2 px-4 md:px-0 py-5 dark:border-light_tert/50 text-center"
          >
